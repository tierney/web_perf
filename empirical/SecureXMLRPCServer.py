...
CERTFILE='/etc/apache2/ssl/selfsigned.crt'  # Replace with your PEM formatted certificate file
# Configure above
...
class SecureXMLRPCServer(BaseHTTPServer.HTTPServer,
  ...
  def __init__(self, server_address, HandlerClass, logRequests=True):
    ...
    It it very similar to SimpleXMLRPCServer but it uses HTTPS for transporting
    XML data.
    """
    self.logRequests = logRequests

    SimpleXMLRPCServer.SimpleXMLRPCDispatcher.__init__(
      self, allow_none = False, encoding = 'UTF-8')
...
class SecureXMLRpcRequestHandler(SimpleXMLRPCServer.SimpleXMLRPCRequestHandler):
  ...
  def do_POST(self):
    ...
    It was copied out from SimpleXMLRPCServer.py and modified to shutdown the
    socket cleanly.
    """
